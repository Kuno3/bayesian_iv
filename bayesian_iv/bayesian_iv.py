...
class bayesian_iv():
    ...
    def theta_sampler(self, G, gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t, step_size, num_step):
        ...
        lp_old = self.log_posterior(G, gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t)
        lp_new = self.log_posterior(G, gamma_at_new, gamma_nt_new, beta_at_new, beta_nt_new, beta_co_c_new, beta_co_t_new)
        threshold = np.log(np.random.rand())
        if threshold < lp_new - sum(r**2) / 2 - lp_old + sum(r_init**2) / 2:
            return gamma_at_new, gamma_nt_new, beta_at_new, beta_nt_new, beta_co_c_new, beta_co_t_new, lp_new
        else:
            return gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t, lp_old
    ...
    def sampling_hmc(
        ...
        for i in tqdm(range(burn_in+num_samples)):
            for _ in range(thinning):
                G = self.G_sampler(gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t)
                gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t, lp = self.theta_sampler(G, gamma_at, gamma_nt, beta_at, beta_nt, beta_co_c, beta_co_t, step_size, num_step)
